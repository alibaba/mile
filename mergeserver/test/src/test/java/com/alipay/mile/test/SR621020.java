 ...
 * 
 * @author xiaoju.luo
 * @version $Id: SR621020.java,v 0.1 2012-11-2 下午02:16:45 xiaoju.luo Exp $
 */
@RunWith(SpecRunner.class)
@Feature("一般条件下的聚合快速查询")
public class SR621020 extends DocdbTestTools {
    /** 超时 */
    ...
    @Tester("xiaoju.luo")
    public void TC621020() {
        stepInfo("执行查询");
        String sql = "select count(distinct TEST_ID) within (GMT_TEST > ?)  as a1, count(*) within(GMT_TEST > ?) as a2, sum(GMT_TEST) within(GMT_TEST>?) as a3 from TEST_DAILY indexwhere TEST_IP=? where TEST_ID=?";
    ...
    @Priority(PriorityLevel.HIGHEST)
    @Tester("xiaoju.luo")
    public void TC621022() {
        stepInfo("执行查询");
        String sql = "select variance(GMT_TEST) within(GMT_TEST > ?),squaresum(GMT_TEST) within(GMT_TEST>?),stddev(GMT_TEST) within(GMT_TEST>=?),avg(GMT_TEST) within(GMT_TEST>?) ,sum(GMT_TEST) within(GMT_TEST>?),min(GMT_TEST) within(GMT_TEST>?) , max(GMT_TEST) within(GMT_TEST>?) ,count(GMT_TEST) within(GMT_TEST>?) , count(*) within(GMT_TEST>?) ,count(distinct GMT_TEST) within(GMT_TEST>?)  from TEST_DAILY indexwhere TEST_IP=? where TEST_ID=?";
        Object[] params = new Object[12];
        params[0] = Long.valueOf(1);
        params[1] = Long.valueOf(1);
        params[2] = Long.valueOf(0);
